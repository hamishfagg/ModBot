...
class Module():
    ...
    def cmdOmegle(self, user, channel, args):
        if self.clients.get(channel, None) == None: # There's no session going on in this channel yet
            if len(args) == 0:
                ...
                self.clients[channel] = {'mode': 'single', 'clients': [Omegle.Omegle(self, channel, 0)], 'running': True, 'connected': False}

            elif args[0] == 'spy':
                self.logger.log(LOG_DEBUG, "Starting Omegle in spy mode, channel %s" % channel)
                self.clients[channel] = {'mode': 'spy', 'clients': [Omegle.Omegle(self, channel, 0), Omegle.Omegle(self, channel, 1)], 'running': True, 'connected': False}
                ...
                self.logger.log(LOG_DEBUG, "Starting Omegle in spy mode, channel %s" % channel)
                self.clients[channel] = {'mode': 'party', 'clients':[], 'running': True, 'count': 4, 'connected': False}
    ...
    def cmdKick(self, user, channel, args):
        try:
            ...
            if channel in self.clients and len(args) > 0: # An index was given
                ...
                self.clients[channel]['connected'] = False
                self.ready = False
    ...
    def cmdCap(self, user, channel, args):
        if len(args) > 0:
            self.clients[self.captcha[0]]['clients'][self.captcha[1]].sendCaptcha(" ".join(args))
    ...
    def on_connected(self, channel, index):
        ...
        print "YAYAYAYAYAYAYAYAYAYAYAYAYAYAY"
        self.clients[channel]['connected'] = True
        self.logger.log(LOG_DEBUG, "OMEGLE: All clients connected for channel %s" % channel)

        for chan in self.clients:
            if not self.clients[chan]['connected']:
                print "connecting for channel %s" % chan
                if connectNext(chan): return
        #all channels are connected
        self.ready = True # any channel is now free to connect a new client

    def connectNext(self, channel):
        if self.captcha == None:
            for client in self.clients[channel]['clients']:
                if client.id == None:
                    print "CONNECTING CLIENT ON %s" % channel
                    client.connect()
                    return True
            return False
